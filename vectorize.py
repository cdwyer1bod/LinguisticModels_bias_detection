...
columns = ["bias_word", "neutral_words", "bias_sent", "neutral_sent"]
...
def get_bias_index(x):
    ...
    tokenized_text = word_tokenize(x['bias_sent'])
    try:
        return int(tokenized_text.index(x['bias_word']))
    except:
...
def get_corpus_freqs(df):
    ...
    for row in df['bias_sent'].values:
...
def get_freqs(filename):
    ...
    word_freq_dist = data['bias_word'].value_counts().astype(float)
